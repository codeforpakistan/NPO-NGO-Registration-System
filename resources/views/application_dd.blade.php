...
<hr>
					...
					<div class="row">
						<div class="col-12 text-center mt-3">
							<button id="reverse_btn_dd" class="btn btn-danger" data-basic-info-id="{{ $detail->basic_info_id }}" data-user-id="{{ session('id')}}">Reverse Application</button>
							<button id="submit_btn_dd" class="btn btn-primary" data-basic-info-id="{{ $detail->basic_info_id }}" data-user-id="{{ session('id')}}">Forward Application</button>
					</div>
        ...
        success: function (response) {
            if (response.success) {
                Swal.fire({
                    ...
                    window.location.href = "{{ route('app_summary_dd') }}";
                });
            } else {
                Swal.fire({
                    title: "Error!",
