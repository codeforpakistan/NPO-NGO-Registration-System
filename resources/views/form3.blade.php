...
<head>
    ...
    <title>Progress with Circular Icons</title>
    ...
    <style>
        ...
        .circle.active {
            background: #28a745; /* Green for active steps */
        }
        .step.active span {
            font-weight: bold;
            color: #28a745;
...
<div class="app-content">
					<section class="section">
						...
						<div class="page-header pt-0">
							<h4 class="page-title"><body>
                                <div class="container mt-5">
        <h3 class="text-center">Welcome, {{ $name }}!</h3>
        <div class="stepper">
            @for ($step = 1; $step <= $totalSteps; $step++)
                <div class="step {{ $step <= $currentStep ? 'active' : '' }}">
                    <div class="circle {{ $step <= $currentStep ? 'active' : '' }}">{{ $step }}</div>
                    <span>Step {{ $step }}</span>
                </div>
            @endfor
        </div>
        <p class="text-center">Step {{ $currentStep }} of {{ $totalSteps }}</p>
        <div class="mt-3 text-center">
            <button class="btn btn-primary" {{ $currentStep == 1 ? 'disabled' : '' }}>Back</button>
            <button class="btn btn-success" {{ $currentStep == $totalSteps ? 'disabled' : '' }}>Next</button>
        </div>
