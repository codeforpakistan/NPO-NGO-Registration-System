...
<hr>
					...
					<div class="row">
						<div class="col-12 text-center mt-3">
							<button id="reverse_btn_district" class="btn btn-danger" data-basic-info-id="{{ $detail->basic_info_id }}" data-user-id="{{ session('id')}}">Reverse Application</button>
							<button id="submit_btn_district" class="btn btn-primary" data-basic-info-id="{{ $detail->basic_info_id }}" data-user-id="{{ session('id')}}">Forward Application</button>
					</div>
...
@endif
		...
		<script>
			$(document).ready(function () {
				$('#submit_btn_district').click(function (e) {
    ...
    $.ajax({
        url: "{{ route('forward.app.district') }}",
        type: "POST",
        data: formData,
        processData: false,
        contentType: false,
        success: function (response) {
            if (response.success) {
                Swal.fire({
                    ...
                    window.location.href = "{{ route('app_summary_district') }}";
                });
            } else {
                Swal.fire({
                    title: "Error!",
...
$("#reverse_btn_district").click(function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: "{{ route('reverse.app.district') }}",
        type: "POST",
        data: formData,
        processData: false,
        contentType: false,
